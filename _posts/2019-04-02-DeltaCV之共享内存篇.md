...
// 如果在其它程序中我们已经存放过数据了,而想要取数据,则直接使用*n就是我们要取的数据了哈你
```
...
private:
    ...
    std::string shm_name;
    std::可以看到5s内,订阅器有4次获取的是未更新的值string lock_name;
    std::string data_name;
    std::string update_name;
...
int a[2];
int main() {
    deltaCV::shm_subscriber<int> sub("shm0");
    while(1)
    {
        bool res = sub.get(a);
